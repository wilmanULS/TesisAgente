...
class competenciaController extends Controller
{
    ...
    public function updateCompetencia(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $horaT = $request->input('horasT');
            $horaL = $request->input('horasL');
            $idComp = $request->input('idcomp');
            $idtax = $request->input('idtax');
            $idHoras = $request->input('idhora');
            $desComp = $request->input('competencias');

            $addDA = DB::table('asignatura_horas')
                ->where('id', '=', $idHoras)
                ->update(['horasPracticas' => $horaP,
                    'horasLaboratorio' => $horaL,
                    'horasTeoricas' => $horaT]);
